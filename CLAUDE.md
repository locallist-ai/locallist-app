...
| **Analytics** | PostHog via REST (`lib/analytics.ts`) — no-op unless `EXPO_PUBLIC_POSTHOG_KEY` is set. Behavioral funnel/retention events (#123): `app_opened{days_since_install}` (cold start + foreground), `screen_view{screen}` (normalized route pattern, e.g. `/plan/[id]`, never the literal id), `wizard_step_viewed{step}` + `wizard_abandoned{step}` (wizard funnel), `plan_viewed{source}` (redefined — low-cardinality `PlanViewSource`, NO more `planId`). `follow_started`/`follow_completed` are SERVER-owned (backend emits them with the same distinct_id + flips `PlanMetrics.WasFollowed`); the app does NOT emit them, to avoid double-counting the same PostHog person |
...
| `builder/custom.tsx` | Manual plan builder (voz editorial, Dir A): EditorialTitle/StepSubtitle + city search (icono magnify) + TIER-AWARE duration (free 1–3 + afford. Plus bloqueada 4–14 con upsell `duration_requires_plus`; Plus hasta 14, mismo token de pill que el wizard) + start date → `PrimaryButton` plano → opens `/plan/new` editor. Cierre unificado con import (X `close`) |
| `builder/import-video.tsx` | Import a plan from your OWN travel video OR photo (F2 T5, Plus). **Thin orchestrator (editorial redesign, Dir A)**: owns the attribution inputs (`platform`/`creatorHandle`), consumes `lib/import/useImportUpload` (phase machine + upload/abort/retry) and wires each phase to `components/import/*` (idle / progress / results). Route/file name kept for deep links; only copy changed. Behaviour preserved: REQUIRE-GUARDED picker (`lib/import/native-picker`; no module ⇒ "update needed" notice), per-kind client validation (`lib/import/validate`; images skip duration, 25 MB cap), `mediaKind` analytics, multipart upload with progress + abort-on-unmount (`importVideo`), matched=selectable+preselected candidates / non-matched="not on LocalList", tier-aware day picker + optional name → `createImportPlan` → `/plan/{id}`. Guest→signup gate, free→`import_requires_plus` upsell. **Self-first idle**: 1-tap "Choose a video or photo" (PrimaryButton, platform=self); the third-party ATTRIBUTION UI (platform selector + disclaimer + optional `creatorHandle`, sanitized trim/64, not logged, rides `Import:ThirdPartyEnabled`; 403 `third_party_import_disabled`) is FOLDED behind an opt-in "Add attribution" link. Analytics carry `platform`, never the handle |
...
| `ui/design-system/` | ChoiceChip, EditorialTitle, StepSubtitle, ProgressDots, PrimaryButton (CTA plano sunsetOrange, texto paperWhite, icono MCI opcional, estados pressed/disabled/loading — compartido builder+import) — wizard design system |
| `import/` | Import screen phases (extracted from the `import-video` god-file, editorial redesign): ImportIdle (self-first idle, neutral media icon, EditorialTitle/StepSubtitle, PrimaryButton CTA, folded third-party attribution), ImportProgress (warm progress: EditorialTitle + TypingDots + two honest steps Upload→Analyze + branded bar), ImportResults (candidates + tier-aware day picker + optional name + create footer; owns selection/create state), CandidateRow (single candidate in the paperWhite/sunsetOrange bubble language) |
...
| `map/PlanMap.tsx` | MapLibre map: NUMBERED order pins (1-based position within the shown day, mirrors FollowDaySheet's N/M; active stop highlighted filled sunsetOrange/larger/paperWhite number, rest paperWhite + electricBlue border + deepOcean number), route line, animated camera. Basemap ENV-conditional (`lib/map/tiles.ts`): with `EXPO_PUBLIC_TILES_URL` uses the self-hosted Protomaps style + OSM/Protomaps attribution and drives per-plan OFFLINE packs; without it, OpenFreeMap online (today's default). In `followMode` shows an offline download/status pill |
...
| `import/` | Import-from-video/image helpers (F2 T5). `useImportUpload.ts`: hook dueño de la máquina de fases (`idle`→`uploading`→`analyzing`→`results`) + refs (busy/lastAsset/abort/mounted) + `doUpload`/`chooseVideo`/`retry` (gates, validación, upload multipart con progreso + abort-on-unmount, analytics `mediaKind`/`platform`); lo consume el orquestador `import-video.tsx`. `import-errors.ts`: helpers PUROS compartidos (mapa de código backend→key i18n incl. `import_media_type_mismatch`, `VALIDATION_KEY`, `sanitizeHandle`/`MAX_HANDLE`, `extractCode`). `native-picker.ts`: REQUIRE-GUARDED expo-image-picker (`pickVideo` abre vídeos E imágenes y etiqueta cada asset con `kind` 'video'|'image'; sin el módulo nativo degrada a `unavailable`, nunca crash de arranque — mismo patrón que `trial-reminder/native-module`). `validate.ts`: validación cliente PURA por `kind` (vídeo 150MB/10min mp4·mov·webm; imagen 25MB SIN duración jpg·png·webp·heic) + `resolveUploadMime`. La subida multipart (`importVideo`) y `createImportPlan` viven en `lib/api.ts` |
...
| `analytics.ts` | PostHog REST capture, fire-and-forget `track()` — no-op without `EXPO_PUBLIC_POSTHOG_KEY`. Anon `distinct_id` persistente (UUID en fichero local `analytics_anon_id` — sobrevive reinicios, MUERE con la desinstalación; nunca Keychain, sería un device-id no reseteable) + `$identify` en anon→user; todos los eventos llevan `country` (locale) y `storefront` (caché RevenueCat) cuando hay valor; `trackPlanLimitIfGate403` emite `plan_limit_hit` desde los 403 estructurados de gates Plus (wired en `api.ts`). `AppEvent` union tipa cada evento; `follow_started`/`follow_completed` son SERVER-owned (no se emiten app-side, ver arriba) |
| `analytics/use-app-lifecycle.ts` | Hooks de retención/navegación (#123, montados por el `RootLayout` en `app/_layout.tsx`): `useAppOpenTracking` emite `app_opened{days_since_install}` en cold start Y en cada vuelta a foreground (`AppState → 'active'`), con los días derivados de la primera apertura persistida (`analytics/app-open.ts`); `useScreenTracking` emite `screen_view{screen}` en cada cambio de ruta de Expo Router, normalizando los segmentos dinámicos a un patrón low-cardinality (`analytics/screen-name.ts`, nunca el id literal) |
