...
| `plan/[id].tsx` | Plan detail + editor: PlanCardPager, inline editing via `usePlanEditor`, handles `/plan/new` and builder preview handoff, Follow button. Owner-only **ShareButton** (floating pill top-right, gated by `shouldShowShareButton`): `POST /plans/{id}/share` (idempotente) → native Share sheet con el enlace `https://locallist.ai/p/{shareToken}`; long-press → ActionSheet Compartir/Dejar de compartir → `DELETE /share` con ConfirmModal. Eventos `plan_share_opened`/`plan_share_revoked` (sin ids) |
