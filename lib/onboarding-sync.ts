 ...
 *
 * A guest fills preferences during the W2 onboarding flow (interests, budget)
 * while unauthenticated; they are persisted locally by `onboarding-store`. On
 * the FIRST successful login/registration (`lib/auth` `login()`), the
 * profile-relevant subset is pushed to `PUT /me/profile` and the local prefs are
 * cleared. Interests have no profile field, so they stay local for wizard/chat
 * pre-fill and are simply dropped once a profile write happens.
 *
 ...
 * has that account's `defaultBudgetTier` OVERWRITTEN by the onboarding pick. The
 * backend merge (`ProfileController.cs`) is per-field, so unrelated fields (city /
 * dietary / pace / group) survive, but budget is clobbered by the fresh guest
 * input. This may be intended (the user just expressed fresh intent) or unwanted
 * (it stomps an established preference). Options if we want to change it: only sync
 * a field when the account has it unset, or gate the whole sync on "new account".
 * Do NOT change the mapping/merge below without a decision, it currently always
 * overwrites the mapped fields.
 */
